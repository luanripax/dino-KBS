...
class RuleBasedPlayer(KnowledgeEngine):
    ...
    @Rule(AND(Fact(speed=P(lambda x: x >= 25 and x < 30)),
              Fact(distance=P(lambda x: x < 600)),
              NOT(Fact(action='K_DOWN'))))   
    def jumpVeryFast2(self):
           self.retract(1)
           self.declare(Fact(action='K_UP'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 30)),
              Fact(distance=P(lambda x: x < 700)),
              NOT(Fact(action='K_DOWN'))))   
    def jumpVeryFast3(self):
           self.retract(1)
           self.declare(Fact(action='K_UP'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 10 and x < 20)),
              Fact(distance=P(lambda x: x < 70)),
              Fact(obHeight=P(lambda x: x < 60))))   
    def down(self):
           self.retract(1)
           self.declare(Fact(action='K_DOWN'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 20 and x < 30)),
              Fact(distance=P(lambda x: x < 170)),
              Fact(obHeight=P(lambda x: x < 60))))   
    def down2(self):
           self.retract(1)
           self.declare(Fact(action='K_DOWN'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 30 and x < 35)),
              Fact(distance=P(lambda x: x < 260)),
              Fact(obHeight=P(lambda x: x < 60))))   
    def down3(self):
           self.retract(1)
           self.declare(Fact(action='K_DOWN'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 35 and x < 38)),
              Fact(distance=P(lambda x: x < 290)),
              Fact(obHeight=P(lambda x: x < 60))))   
    def down4(self):
           self.retract(1)
           self.declare(Fact(action='K_DOWN'))

    @Rule(AND(Fact(speed=P(lambda x: x >= 38)),
              Fact(distance=P(lambda x: x < 300)),
              Fact(obHeight=P(lambda x: x < 60))))   
    def down5(self):
           self.retract(1)
